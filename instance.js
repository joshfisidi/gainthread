import { MPCGameGeneratorRuntimeType } from "./type.js";

// Import type for TypeScript-like type checking
const C3 = window.C3;
 ...
 */
const PLUGIN_CLASS = SDK.Plugins.MPC_GameGenerator = class MPCGameGeneratorInstance extends SDK.IInstanceBase {
  constructor(inst, properties) {
    ...
    // Set up DOM message handlers
    this.AddDOMMessageHandler("generation-complete", data => this._onGenerationComplete(data));
    this.AddDOMMessageHandler("generation-error", data => this._onGenerationError(data));
  ...
  async _makeGeneration(prompt, type) {
    if (this._isGenerating) {
      ...
      this.Trigger(C3.Plugins.MPC_GameGenerator.Cnds.OnGenerationError);
      return;
    }
    ...
    this.Trigger(C3.Plugins.MPC_GameGenerator.Cnds.OnGenerationStart);
    
    // Send to DOM side
    try {
      const result = await this.PostToDOMAsync("make-generation", {
        prompt,
        type,
        apiKey: this._apiKey,
      ...
      this.Trigger(C3.Plugins.MPC_GameGenerator.Cnds.OnGenerationComplete);
    } catch (error) {
  ...
  _onGenerationComplete(data) {
    ...
    if (this._generationType === "code") {
      this.Trigger(C3.Plugins.MPC_GameGenerator.Cnds.OnCodeGenerationComplete);
    } else if (this._generationType === "object") {
      this.Trigger(C3.Plugins.MPC_GameGenerator.Cnds.OnObjectGenerationComplete);
    } else if (this._generationType === "sprite") {
      this.Trigger(C3.Plugins.MPC_GameGenerator.Cnds.OnSpriteGenerationComplete);
    }
  }
